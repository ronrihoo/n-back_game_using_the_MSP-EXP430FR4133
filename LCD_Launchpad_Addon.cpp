...
// Using a weird name so that one of the common ones won't be used here
LCD_LAUNCHPAD LCD_XYZ;

// The amount of LCD segments for characters
int segments = 6;                               // note: segment quanitity starts at 1 and NOT 0
...
void scroll_left(char x[], int initial_pause, int scroll_speed, int post_delay, boolean post_clear)
{
  ...
  for (i = 0; i < length; i++)
  {
    ...
    // Allow user to escape (or skip) the message
...
void scroll_right(char x[], int initial_pause, int scroll_speed, int post_delay, boolean post_clear)
{
  ...
  for (i = 0; i < length + 1; i++)
  {
    ...
    if (k >= segments)
    {
      for (j = 0; j < segments; j++)
      {
        LCD_XYZ.showChar(x[k-j], ((segments - 1) - j));
      }
    }
    else
    {
...
int menu_selection(int items)
{
  ...
  while(selection < 1)
  {
    LCD_XYZ.displayText("Sel ", (segments - 5));       // "Sel" for select (extra space to separate from #)
    ...
    LCD_XYZ.showChar(choice, (segments - 1));
